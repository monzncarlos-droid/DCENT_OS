#!/bin/sh
#
# DCENTos post-build script — am2-t17 (BM1397 17-series Zynq am2 variant)
# D-Central Technologies, 2026
#
# 2026-08-27 Antminer 17-Series Complete Unlock Armada (agent B2) — clone of
# board/zynq/am2-s17pro/post-build.sh for T17 (3x30 BHB07701, dsPIC33 G2a per A1 §V1).
...
# There is NO live T17 unit on the D-Central fleet. The BM1397 runtime
...
#
# CRITICAL — :
#   board_target MUST be "am2-t17" here. Sysupgrade tarball prefix is
#   "sysupgrade-am2-t17/". Wrong name = silent failure = brick on flash.
#
set -e
TARGET_DIR=$1
...
[ -r "$ARCHIVE_ADMISSION_SRC" ] || {
    echo "DCENTos post-build (am2-t17): ERROR: archive-admission helper not found at $ARCHIVE_ADMISSION_SRC" >&2
    exit 1
}
...
[ -r "$ZYNQ_GEOMETRY_SRC" ] || {
    echo "DCENTos post-build (am2-t17): ERROR: Zynq geometry helper not found at $ZYNQ_GEOMETRY_SRC" >&2
    exit 1
}
...
if [ -f "$REVERT_S17_SRC" ]; then
    ...
    echo "DCENTos post-build (am2-t17): installed revert_to_stock_s17.sh from scripts/"
else
    echo "DCENTos post-build (am2-t17): WARNING: revert_to_stock_s17.sh not found at $REVERT_S17_SRC" >&2
fi
...
if [ -f "$MANIFEST_SRC" ]; then
    ...
    echo "DCENTos post-build (am2-t17): installed stock-bitmain-manifest.json"
fi
...
if [ -f "$DASHBOARD_SRC" ]; then
    ...
    echo "DCENTos post-build (am2-t17): installed dashboard SPA ($DASHBOARD_SIZE bytes) at /usr/share/dcentos-dashboard/index.html"
    if [ "$DASHBOARD_SIZE" -lt 100000 ]; then
        echo "DCENTos post-build (am2-t17): ERROR: dashboard appears truncated ($DASHBOARD_SIZE bytes < 100 KB floor)" >&2
        echo "  Run: cd DCENT_OS_Antminer/dashboard && npm run build" >&2
        exit 1
    fi
else
    echo "DCENTos post-build (am2-t17): ERROR: dashboard not found at $DASHBOARD_SRC" >&2
    echo "  Run: cd DCENT_OS_Antminer/dashboard && npm run build" >&2
    exit 1
fi
...
if [ -f "$DCENTOS_INIT" ]; then
    ...
    echo "DCENTos post-build (am2-t17): installed dcentos-init as /sbin/init"
else
    echo "DCENTos post-build (am2-t17): WARNING: dcentos-init not found at $DCENTOS_INIT"
...
if [ -f "$DCENTRALD_BIN" ]; then
    ...
    echo "DCENTos post-build (am2-t17): installed dcentrald ($DCENTRALD_SIZE bytes)"
else
    echo "DCENTos post-build (am2-t17): ERROR: dcentrald not found at $DCENTRALD_BIN" >&2
...
if [ ! -f "$STAGED_BIN" ]; then
    echo "DCENTos post-build (am2-t17): ERROR: dcentrald missing from $STAGED_BIN" >&2
    exit 1
fi
...
if [ "$BIN_MTIME" -gt 0 ] && [ "$NOW_EPOCH" -gt "$BIN_MTIME" ]; then
    ...
    if [ "$BIN_AGE_HOURS" -gt 12 ]; then
        echo "DCENTos post-build (am2-t17): WARN: dcentrald binary is ${BIN_AGE_HOURS}h old." >&2
...
if command -v md5sum > /dev/null 2>&1; then
    ...
    echo "DCENTos post-build (am2-t17): md5 $(cat "${TARGET_DIR}/etc/dcentos/dcentrald.md5")"
fi
...
# -----------------------------------------------------------------------------
# am2-t17 rides the am2-s17pro baked default (same BM1397 am2 safety
# envelope: fan_max_pwm<=30, voltage_mv<=14500, mining.enabled=false) with
# ONLY the per-model geometry patched: serial_chip_count = 30
# (3x30 T17, A2 driver-facts table) and model = "t17"
...
if [ -f "$S17_AM2_BAKED_CFG" ]; then
    ...
    ' "$S17_AM2_BAKED_CFG" > "${TARGET_DIR}/etc/dcentrald.toml"; then
        echo "DCENTos post-build (am2-t17): installed am2-s17pro baked default patched to serial_chip_count=30 model=t17 as /etc/dcentrald.toml"
    else
        echo "DCENTos post-build (am2-t17): ERROR: per-model baked-config patch failed" >&2
        exit 1
    fi
else
    echo "DCENTos post-build (am2-t17): ERROR: S17 baked default not found at:" >&2
...
if [ -f "$BAKED_RUNTIME_CFG" ]; then
    ...
    if [ -n "$BAKED_VOLTAGE_MV" ] && [ "$BAKED_VOLTAGE_MV" -gt 14500 ] 2>/dev/null; then
        echo "DCENTos post-build (am2-t17): ERROR: baked dcentrald.toml has mining.voltage_mv=${BAKED_VOLTAGE_MV} > 14500 mV am2 ceiling" >&2
    ...
    if [ -n "$BAKED_FAN_MAX" ] && [ "$BAKED_FAN_MAX" -gt 30 ] 2>/dev/null; then
        echo "DCENTos post-build (am2-t17): ERROR: baked dcentrald.toml has thermal.fan_max_pwm=${BAKED_FAN_MAX} > 30" >&2
    ...
    echo "DCENTos post-build (am2-t17): baked default voltage_mv=${BAKED_VOLTAGE_MV:-unset} fan_max_pwm=${BAKED_FAN_MAX:-unset} (within am2 EE safety envelope)"
fi
...
echo "am2-t17"    > "${TARGET_DIR}/etc/dcentos/board_target"
...
if [ -n "${DCENT_RELEASE_PUBKEY_FILE:-}" ]; then
    if [ ! -f "${DCENT_RELEASE_PUBKEY_FILE}" ]; then
        echo "DCENTos post-build (am2-t17): ERROR: release public key not found at ${DCENT_RELEASE_PUBKEY_FILE}" >&2
        exit 1
    fi
    ...
    echo "DCENTos post-build (am2-t17): embedded release public key"
elif [ "${DCENT_REQUIRE_RELEASE_KEY:-0}" = "1" ]; then
    echo "DCENTos post-build (am2-t17): ERROR: DCENT_REQUIRE_RELEASE_KEY=1 but DCENT_RELEASE_PUBKEY_FILE is not set" >&2
    exit 1
else
    echo "DCENTos post-build (am2-t17): WARNING: no release public key embedded (lab-only image)"
fi

echo "DCENTos post-build (am2-t17): directories, permissions, and board identity set (EXPERIMENTAL desk-validated scaffold — no live T17 on the fleet)."
