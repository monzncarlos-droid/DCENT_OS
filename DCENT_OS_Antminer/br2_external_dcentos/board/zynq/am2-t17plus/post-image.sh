#!/bin/sh
#
# DCENTos post-image script — am2-t17plus (T17+ Zynq am2 17-series variant)
...
#
# CRITICAL — :
#   BOARD_NAME must be "am2-t17plus" (not any sibling 17-series target).
...
BOARD_NAME="am2-t17plus"
BOARD_FAMILY="am2"
OUTPUT_TAR="${BINARIES_DIR}/dcentos-sysupgrade-am2-t17plus.tar"

echo "=== DCENTos Post-Image Builder (am2-t17plus) ==="
...
if [ ! -f "$ROOTFS" ]; then
    ...
    echo "  Enable BR2_TARGET_ROOTFS_SQUASHFS=y in the am2-t17plus defconfig." >&2
    exit 1
fi
...
infer_package_version() {
    ...
    for candidate in \
        ...
        "${PROJECT_ROOT}/br2_external_dcentos/board/zynq/am2-t17plus/rootfs-overlay/etc/dcentos-version" \
...
/ {
    description = "DCENT_OS T17+ model-bound UBI kernel FIT";
    #address-cells = <1>;

    images {
        kernel@1 {
            description = "DCENT_OS T17+ UBI kernel";
    ...
    configurations {
        default = "config@1";
        config@1 {
            description = "DCENT_OS T17+ kernel plus exact 17-family DTB";
            kernel = "kernel@1";
            fdt = "fdt@1";
        };
    };
...
=== DCENTos am2-t17plus Build Info (EXPERIMENTAL package) ===
Build date: $(date -u +"%Y-%m-%d %H:%M:%S UTC")
Board:      ${BOARD_NAME} (T17+ Zynq am2 17-series variant)
...
Target fleet:
  (none — no live T17+ unit)
EOF

echo ""
echo "=== Build Complete (am2-t17plus — EXPERIMENTAL package, no live proof) ==="
