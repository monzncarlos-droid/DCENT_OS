#!/bin/sh
#
# DCENTos post-image script — am1-s9k (Antminer S9k, Zynq am1 family)
# D-Central Technologies, 2026.
#
# 2026-08-29 S9k/S9 SE packaging-lane wave — the 2026-08-28 full-line
# campaign's missing S9k install artifact body. The held S9k binaries
# (cgminer + bitmain_axi.ko + poweroff_hashboard) are the am1 Zynq FPGA
# stack — the am1-s9 board family — so this lane reuses the proven am1-s9
# SoC boot inputs (vendor/soc-boot/s9) and the shared Zynq packaging
# helpers. S9k stays a DISTINCT 3x60 interval-4 identity (not S9 SE).
#
# ## PACKAGE-ONLY / INSTALL DENIED ############################################
# The BoardDesc row (am1-s9k, BM1393, 3x60 interval 4) stays
...
BOARD_NAME="am1-s9k"
BOARD_FAMILY="am1"
OUTPUT_TAR="${BINARIES_DIR}/dcentos-sysupgrade-am1-s9k.tar"

echo "=== DCENTos Post-Image Builder (am1-s9k) ==="
echo "    am1 Zynq S9-family boot inputs; package-only/install-denied (S9k distinct identity)."
echo ""
...
if [ ! -f "$ROOTFS" ]; then
    ...
    echo "  Enable BR2_TARGET_ROOTFS_SQUASHFS=y in the am1-s9k defconfig." >&2
    exit 1
fi
...
# geometry helper has no am1-s9k profile, and a package-only-denied
...
infer_package_version() {
    ...
    for candidate in \
        ...
        "${PROJECT_ROOT}/br2_external_dcentos/board/zynq/am1-s9k/rootfs-overlay/etc/dcentos-version" \
...
if [ -n "${DCENT_EXTRACTIONS_DIR:-}" ] && [ -d "$DCENT_EXTRACTIONS_DIR" ]; then
    ...
    echo "ERROR: No SoC boot inputs for am1-s9k. Set \$DCENT_EXTRACTIONS_DIR or populate vendor/soc-boot/s9/ (see vendor/soc-boot/README.md)." >&2
    exit 1
fi
...
# -----------------------------------------------------------------------------
# Stage sysupgrade-am1-s9k/ and build tarball
# -----------------------------------------------------------------------------
STAGING=$(mktemp -d)
...
=== DCENTos am1-s9k Build Info (RESEARCH SHELL package) ===
Build date: $(date -u +"%Y-%m-%d %H:%M:%S UTC")
Board:      ${BOARD_NAME} (Antminer S9k, Zynq am1 family)
...
Target fleet:
  (none — no live S9k unit; the S9k-vs-S9-SE carrier question stays open until a same-unit receipt)
EOF

echo ""
echo "=== Build Complete (am1-s9k — RESEARCH SHELL package, install denied) ==="
