#!/usr/bin/env python3
"""Offline, evidence-derived S19j Pro complete-enablement workflow controller.
...
The campaign spans every Antminer S19j Pro "version" D-Central can reach --
XIL (Zynq), BB (AM335x BeagleBone), Amlogic (am3-s19jpro-aml), S19j Pro+
(am3-s19jproplus), and CV1835 control boards -- from locked stock firmware
through unlock, staged bounded mining, endurance, persistent install, and
acceptance, mirroring the proven S19k office-gauntlet campaign pattern.
"""
...
DEFAULT_EVIDENCE_ROOT = REPO_ROOT / ".s19jpro-enablement-evidence"
...
def load_module(path: Path, label: str) -> Any:
    name = f"s19jpro_workflow_{label.replace('-', '_')}"
...
def evaluate(
    ...
    return {
        "schema": "dcentos.s19jpro-enablement-status/v1",
...
def emit_agent_tasks(
    ...
    return {
        "schema": "dcentos.s19jpro-agent-wave/v1",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=(
            Path(os.environ["DCENT_S19JPRO_ARTIFACT_ROOT"])
            if os.environ.get("DCENT_S19JPRO_ARTIFACT_ROOT")
            else None
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"S19JPRO_WORKFLOW_REFUSED: {error}", file=sys.stderr)
        return 1
    ...
    if args.command == "emit-agent-tasks":
        ...
        output = {
            "schema": "dcentos.s19jpro-next/v1",
            "campaign_id": report["campaign_id"],
